...
minnargin = 4; % check number of inputs
maxnargin = 14;
...
elseif length(lambda_psi) == R
    ...
    error('GLMM_ADMM:wrong_size_regularization','lambda_psi must be a scalar or a R-dimensional vector')
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% EM tensors nr * nc * L, for i=1...P
M = zeros(L,R,N);
[nr,nc,~] = size(EMs_tensor{1});
...
for p=1:R
    ...
    for ii=1:nc
        for jj=1:nr
            M(:,p,idxx) = permute(EMs_tensor{p}(jj,ii,:),[3 2 1]);
            idxx = idxx + 1;
        end
    end
...
%             parfor p = 1:R
            ...
            if iter < maxiter_admm
                if norm(primal(iter)) > nu * norm(dual(iter))
                    ...
                    A = A/tau_incr;
% % % % % % % % % % % % % % % %                     d1 = tau_incr*d1; d2 = tau_incr*d2; d3 = tau_incr*d3; d4 = tau_incr*d4; 

                elseif norm(dual(iter)) > nu * norm(primal(iter))
                    rho(iter+1) = rho(iter)/tau_decr;
                    A = tau_decr*A;
% % % % % % % % % % % % % % % %                     d1 = d1/tau_decr; d2 = d2/tau_decr; d3 = d3/tau_decr; d4 = d4/tau_decr; 
                else
                    rho(iter+1) = rho(iter);
                end
            end
